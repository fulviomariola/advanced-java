...
public class UserDaoImplTest {
	...
	private List<User> getUserInRange(int minId, int maxId) throws SQLException {
	...
	public void testFindAndUpdate() throws SQLException {
		...
		user.setName("abcde");
		user.setPassword("sfghg");
		
		userDao.update(user);
		
		retrievedUserOpt = userDao.findById(maxId);
		
		assertTrue("no update user retrieved", retrievedUserOpt.isPresent());
		
		retrievedUser = retrievedUserOpt.get();
		
		assertEquals("retrieved user doesn't match update user", user, retrievedUser);
	...
	public void testSaveMultiple() throws SQLException {
		...
		var retrievedUsers = getUserInRange((maxId - users.size()) + 1, maxId);
	...
	public void testDelete() throws SQLException {
		...
		var retrievedUsers = getUserInRange((maxId - NUM_TEST_USERS) + 1, maxId);
